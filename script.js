const STORAGE_KEY = "filmshelf_movies_v1";
  ...
  frame.innerHTML = `
    <div class="frame-inner">
      ...
      <div class="meta">
        ...
        <button data-toggle="${movie.id}">${actionText}</button>
      </div>
    </div>
  `;
...
posterInput.onchange = () => {
  ...
  reader.onload = (event) => {
    ...
    img.onload = () => {
      const orientationClass = img.height >= img.width ? "is-vertical" : "is-horizontal";

      imagePreview.innerHTML = `
        <img class="${orientationClass}" src="${selectedPoster}" alt="Selected poster">
      `;
    };
